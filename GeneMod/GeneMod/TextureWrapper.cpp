...
TextureWrapper::TextureWrapper(Renderer* renderer)
	:
	renderer(renderer)
{
	//Initialize
...
bool TextureWrapper::loadFromFile(std::string path)
{
	...
	else
	{
		...
		newTexture = SDL_CreateTextureFromSurface(renderer->GetgRenderer(), loadedSurface);
...
void TextureWrapper::render(int x, int y, SDL_Rect* clip, double angle, SDL_Point* center, SDL_RendererFlip flip)
{
	...
	//Render to screen
	SDL_RenderCopyEx(renderer->GetgRenderer(), mTexture, clip, &renderQuad, angle, center, flip);
}
